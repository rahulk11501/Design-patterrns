...
```
So we have overwritten the getc and eof? methods in such a way that, we can use a `String` as if it were an open file (it only implements a small part of the `IO` interface, essentally what we need).
